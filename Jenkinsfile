        ...
        success {
            script {
                ...
                withCredentials([usernamePassword(credentialsId: 'd17d7c30-12bf-44d2-88f8-e9f3814e43f2', usernameVariable: 'USER', passwordVariable: 'PASSWORD')]) {
                    sh "curl -u $USER:$PASSWORD -X POST -d '{\"body\": \"Ok for the commit\", \"commit_id\": \"${repository_commit_id}\"}' \"https://api.github.com/repos/${repository_name}/pulls/${repository_commit_id}/reviews\""
                }
            }
        }
    }
